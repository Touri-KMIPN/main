...
async function fetchSpots(sessionId: string, onFound: (spots: Spot[]) => void) {
  const spots = await $dexie.spots.where("sessionId").equals(sessionId).toArray();
  onFound(spots);
}
...
export default function Page({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  return (
    <Conversation initialMessage={initialMessage ?? null} messages={messages} setMessages={setMessages} chatSessionId={sessionId} />
  )
}
